import React, { useState, useCallback } from "react";
import { Table, Button, Form, InputGroup } from "react-bootstrap";
import { useNavigate, Link } from "react-router-dom";
import { Search, Plus, RotateCcw } from "lucide-react";
import { doctorAPI } from "../../services/api";
import useCRUD from "../../hooks/useCRUD";
import CardContainer from "../../components/CardContainer";

export default function Doctors({ search: topSearch = "" }) {
  const navigate = useNavigate();
  const { items, loading, error, reload } = useCRUD(useCallback(() => doctorAPI.getAll(), []));
  const [searchKeyword, setSearchKeyword] = useState("");

  const handleSearch = (e) => setSearchKeyword(e.target.value);

  const filtered = items.filter(d => {
    const searchStr = `${d.firstName} ${d.lastName} ${d.specialization} ${d.department} ${d.code || ""} ${d.id}`.toLowerCase();
    const matchesSearch = !searchKeyword ? true : searchStr.includes(searchKeyword.toLowerCase());
    const matchesTopSearch = !topSearch ? true : searchStr.includes(topSearch.toLowerCase());
    return matchesSearch && matchesTopSearch;
  });

  const resetFilters = () => {
    setSearchKeyword("");
    reload();
  };

  return (
    <CardContainer>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h4 className="fw-bold mb-0">Doctors</h4>
          <p className="text-muted small mb-0">Medical staff inventory · {items.length} records</p>
        </div>
        <div className="d-flex gap-2 align-items-center">
          <Button variant="link" className="p-0 text-dark text-decoration-none d-flex align-items-center gap-1" onClick={resetFilters}>
            <RotateCcw size={14}/> Reset
          </Button>
          <Button as={Link} to="/doctors/create" variant="primary" className="d-flex align-items-center gap-1">
            <Plus size={16}/> Add Doctor
          </Button>
        </div>
      </div>

      <div className="d-flex justify-content-between align-items-center mb-3">
        <div style={{ maxWidth: "400px", width: "100%" }}>
          <InputGroup size="sm">
            <InputGroup.Text className="bg-white border-end-0">
              <Search size={14} className="text-muted"/>
            </InputGroup.Text>
            <Form.Control
              placeholder="Search doctors by name, specialization, dept..."
              className="border-start-0"
              value={searchKeyword}
              onChange={handleSearch}
            />
          </InputGroup>
        </div>
      </div>

      <div className="table-responsive">
        <Table hover className="align-middle">
          <thead className="bg-light">
            <tr>
              <th>ID</th>
              <th>Full Name</th>
              <th>Specialization</th>
              <th>Department</th>
              <th>Contact</th>
              <th>Status</th>
              <th className="text-center">Actions</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={7} className="text-center py-4">Loading doctors...</td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={7} className="text-center py-4 text-muted">No doctors found.</td></tr>
            ) : filtered.map(d => (
              <tr key={d.id}>
                <td className="fw-bold text-primary">{d.code || "N/A"}</td>
                <td>
                  <div className="fw-bold">{d.firstName} {d.lastName}</div>
                  <div className="text-muted small">License: {d.licenseNumber}</div>
                </td>
                <td><span className="badge bg-info text-dark opacity-75">{d.specialization}</span></td>
                <td>{d.department}</td>
                <td>
                  <div className="small">{d.email}</div>
                  <div className="small text-muted">{d.phone}</div>
                </td>
                <td>
                  <span className={`badge ${d.available ? "bg-success" : "bg-danger"}`}>
                    {d.available ? "Available" : "On Leave"}
                  </span>
                </td>
                <td className="text-center">
                  <Button 
                    variant="link" 
                    className="p-0 text-primary me-2" 
                    onClick={() => navigate(`/doctors/${d.id}`)}
                    title="View & Edit"
                  >
                    <i className="bi bi-eye-fill fs-5"></i>
                  </Button>
                </td>
              </tr>
            ))}
          </tbody>
        </Table>
      </div>
    </CardContainer>
  );
}
